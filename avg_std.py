...
print(f"Selected features: {len(base_features)}")
print(f"Total output columns: {len(base_features) * 2 + 1} (mean + std + Patch)\n")
...
for file in txt_files:
    ...
    with open(file, 'r', encoding='utf-8') as f_in, \
        ...
        try:
            ...
            print("Missing 'Parent' column, file skipped")
            continue
        ...
        for feat in base_features:
            try:
                ...
                print(f"Feature '{feat}' not found, skipped")

        if not feature_indices:
            print("No valid features found, file skipped")
            continue
    ...
    with open(out_path, 'r', encoding='utf-8') as f_check:
        ...
        print(f"Number of columns: {len(csv_header)} (1 Patch + {len(feature_indices)} mean + {len(feature_indices)} std)\n")
