...
from utils import make_weights_for_balanced_classes_split
...
def train_clam_cv(
    ...
    embed_dim = first_sample.shape[1]
    print(f"[INFO] Auto-detected feature dimension: {embed_dim}")
    ...
    for fold, (train_idx, val_idx) in enumerate(split_iter):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for bags, labels in tqdm(train_loader):
                bags = bags.to(device)
                labels = labels.to(device)
                
                # Map to soft labels
                if labels.item() == 2: # Biphasic
                    target = torch.tensor([[0.5, 0.5, 0.0]]).to(device)
                elif labels.item() == 0: # Epithelioid
                    target = torch.tensor([[1.0, 0.0, 0.0]]).to(device)
                else: # Sarcomatoid
                    target = torch.tensor([[0.0, 1.0, 0.0]]).to(device)

                logits, _, Y_hat, _, results = model(bags[0], label=labels, instance_eval=True)
                loss = soft_cross_entropy(logits, target) + results['instance_loss']
